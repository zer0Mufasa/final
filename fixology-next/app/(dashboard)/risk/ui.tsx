        ...
        <div className="lg:col-span-2 space-y-4">
          <GlassCard className="p-6 rounded-3xl border-red-400/30 bg-red-500/5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="text-sm font-semibold text-[var(--text-primary)]">High-risk tickets</div>
            </div>
            <div className="space-y-3">
              {highRiskTickets.map((t) => (
                ...
                >
                  ...
                  <div className="text-xs text-[var(--text-primary)]/60">{t.customerName} • {t.device}</div>
          ...
          <GlassCard className="p-6 rounded-3xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="text-sm font-semibold text-[var(--text-primary)]">Repeat customers</div>
            </div>
            <div className="space-y-2">
              {repeatCustomers.map((name) => (
                <div key={name} className="rounded-2xl bg-white/[0.03] border border-white/10 p-3">
                  ...
                  <div className="text-xs text-[var(--text-primary)]/60 mt-1">3+ repairs in last 6 months</div>
                </div>
              ))}
            </div>
          </GlassCard>
        ...
        <div className="px-4 py-5 sm:p-6 space-y-6">
          <GlassCard className="p-6 rounded-3xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="text-sm font-semibold text-[var(--text-primary)]">Risk summary</div>
            </div>
            <div className="space-y-4">
              <div>
                <div className="text-xs text-[var(--text-muted)] mb-1">High risk tickets</div>
                <div className="text-2xl font-bold text-red-300">{highRiskTickets.filter((t) => t.risk === 'high').length}</div>
              </div>
              <div>
                <div className="text-xs text-[var(--text-muted)] mb-1">Medium risk</div>
                <div className="text-2xl font-bold text-yellow-300">{highRiskTickets.filter((t) => t.risk === 'medium').length}</div>
              </div>
              <div>
                <div className="text-xs text-[var(--text-muted)] mb-1">Repeat customers</div>
                <div className="text-2xl font-bold text-[var(--text-primary)]">{repeatCustomers.length}</div>
              </div>
            </div>
          </GlassCard>

          <GlassCard className="p-6 rounded-3xl">
            <div className="text-sm font-semibold text-[var(--text-primary)] mb-3">Recommended actions</div>
